...
const executeCustomAction = tool(
    async (input): Promise<string> => {
      try {
        setLastEvalError(null); // Reset global errors
  
        const TIMEOUT_MS = 10000; // Timeout di 10 secondi
        ...
        const userCodePromise = asyncWrapperFn(require, vec3);
        const timeoutPromise = new Promise<string>((_, reject) =>
          setTimeout(() => reject(new Error("Azione annullata per timeout (10s).")), TIMEOUT_MS)
        );
  
        const result = await Promise.race([userCodePromise, timeoutPromise]);
        ...
        return `<TASK_FAILED>${error.message?.includes("timeout") 
          ? "Errore: tempo massimo superato. L'azione è stata interrotta." 
          : "Errore interno nell'esecuzione del tool: " + error.stack
        }</TASK_FAILED>`;
      }
    },
  {
